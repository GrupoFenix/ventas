...
class CtrlCarrito extends Controlador {
    public function agregar() {
        ...
        if (is_object($_SESSION['carrito'])){
            $_SESSION['carrito']->agregar($_GET['idlibros'],1);
            
            if(isset($_GET['url'])){
                switch($_GET['url']){
                    case 'detalles':
                        header("Location: ?ctrl=CtrlLibro&accion=verDetalles&idlibros=".$_GET['idlibros']);
                        exit();
                        break;
                    case 'carrito':
                        $this->mostrar();
                        ...
                        break;
                    default:
                        header("Location: ?ctrl=CtrlLibro&accion=getCatalogo&idlibros=".$_GET['idlibros']);
                        exit();
                }
            }else{
                header("Location: ?ctrl=CtrlLibro&accion=getCatalogo&idlibros=".$_GET['idlibros']);
    ...
    public function sacar() {
        if (isset($_SESSION['carrito'])){
            ...
            $_SESSION['carrito']->sacar($_GET['idlibros'],$cant);
        }
